...
const baseUrl = env.NODE_ENV === 'production' ? env.PUBLIC_SERVER_URL : 'http://localhost:2022'

const auth = initAuth({
  db,
  webUrl: env.PUBLIC_CLIENT_URL,
  baseUrl: baseUrl,
  productionUrl: env.PUBLIC_SERVER_URL,
  apiPath: env.PUBLIC_SERVER_API_PATH,
  additionalTrustedOrigins: env.CORS_ORIGIN,
...
export function createServer(): {
  ...
  server.route({
    method: ['GET', 'POST'],
    url: `${env.PUBLIC_SERVER_API_PATH}/auth/*`,
