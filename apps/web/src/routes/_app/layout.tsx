...
export const Route = createFileRoute('/_app')({
  // no need for zodValidator/fallback @see https://github.com/TanStack/router/issues/4322
...
function RouteComponent() {
   ...
   * 4. Refetch on useSession hook is triggered
   * 5. Navigate replaces URL, removing the `refetchSession` param
   * 6. Due to replace: true, the component isn't actually remounted so we are not re-rendering unnecessarily
   */
  const onRefetchSession = React.useEffectEvent(async () => {
    ...
    // Trigger useSession hook to refetch
